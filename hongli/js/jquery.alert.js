(function($){
	$.alerts={
	 ...
	 prompt:function(message,title,value,callback){
		...
		$.alerts._show(title,message,value,400,300,'prompt',function(result){
			if(callback)callback(result);
		});
	 },
...
$.alerts._show(title,message,null,width,height,'choose',function(result){if(callback)callback(result);});},
html:function(message,title,width,height){if(title==null)title='提示';
$.alerts._show(title,message,null,width,height,'html',null);},
	...
	_show:function(title,msg,value,width,height,type,callback){
		...
		$("BODY").append('<div id="popup_container" class="gSys_msg_box">'+
			...
			'<div id="popup_opt_box" class="popt"><div class="opt" id="opt"></div></div>'+'</div>');
		...
		$("#popup_container").css({position:'absolute',zIndex:99999,padding:0,margin:0,width:width+'px'});
		if(height){
		...
		switch(type){
			case'alert':
				...
				$("#popup_ok").mouseover(function(){$(this).attr("class","pbtn btn_hover");});
				$("#popup_ok").mouseout(function(){$(this).attr("class","pbtn");});
				$("#popup_ok").mousedown(function(){$(this).attr("class","pbtn btn_active");});
				$("#popup_ok").mouseup(function(){$(this).attr("class","pbtn");});
				$("#popup_ok").focus().keypress(function(e){if(e.keyCode==13||e.keyCode==27)$("#popup_ok").trigger('click');});
				break;

			case'confirm':
				$("#opt").html('<div id="popup_cancel" class="pbtn" tabindex="0"><span>'+
					$.alerts.cancelButton+'</span></div><div id="popup_ok" class="pbtn" tabindex="0"><span>'+
				...
				$("#popup_ok, #popup_cancel").mouseover(function(){$(this).attr("class","pbtn btn_hover");});
				$("#popup_ok, #popup_cancel").mouseout(function(){$(this).attr("class","pbtn");});
				$("#popup_ok, #popup_cancel").mousedown(function(){$(this).attr("class","pbtn btn_active");});
				$("#popup_ok, #popup_cancel").mouseup(function(){$(this).attr("class","pbtn");});
				$("#popup_ok").focus();
				...
				case'iframe':
					...
					$("#popup_container").append("<div id='popup_iframe_box' class='piframe'><div id='loading' class='loading'><img src='../images/alert/loading.gif'></div></div>");
				...
				case'prompt':
					...
					$("#popup_message").append('<br /><input type="password" class="pinput" size="20" maxlength="30" id="popup_prompt" />');
					$("#opt").html('<div id="popup_cancel" class="pbtn" tabindex="0"><span>'+
					$.alerts.cancelButton+'</span></div><div id="popup_ok" class="pbtn" tabindex="0"><span>'+
					$.alerts.okButton+'</span></div>');
					...
					$("#popup_ok, #popup_cancel").mousedown(
						function(){$(this).attr("class","pbtn btn_active");
					});
				...
				case'choose':$("#popup_message_box b").remove();
$("#popup_message").attr("class","cp");
$("#opt").html('<div id="popup_cancel" class="pbtn" tabindex="0"><span>'+
$.alerts.cancelButton+'</span></div><div id="popup_ok" class="pbtn" tabindex="0"><span>'+
$.alerts.okButton+'</span></div>');
...
$("#popup_ok, #popup_cancel").mousedown(function(){$(this).attr("class","pbtn btn_active");});
$("#popup_ok, #popup_cancel").mouseup(function(){$(this).attr("class","pbtn");});
$("#popup_ok").click(function(){var val=$("#popup_message");
$.alerts._hide();if(callback)callback(val);});
$("#popup_cancel").click(function(){$.alerts._hide();if(callback)callback(null);});
$("#popup_ok, #popup_cancel").keypress(function(e){if(e.keyCode==13)$("#popup_ok").trigger('click');if(e.keyCode==27)$("#popup_cancel").trigger('click');});break;
case'html':$("#popup_message_box b").remove();
$("#popup_opt_box").remove();break;}

if($.alerts.draggable){try{$("#popup_container").easydrag();
$("#popup_container").setHandler('popup_title');$("#popup_title").css({cursor:'move'});}catch(e){}}},_hide:function(){$("#popup_container").remove();
$.alerts._overlay('hide');$.alerts._maintainPosition(false);},_overlay:
function(status){
switch(status){
case'show':
...
$.alerts.horizontalOffset;if(top<0)top=0;if(left<0)left=0;
top=top+$(window).scrollTop();$("#popup_container").css({top:top+'px',left:left+'px'});
$("#popup_overlay").height($(document).height());},_maintainPosition:function(status){
if($.alerts.repositionOnResize){
switch(status){
case true:
$(window).bind('resize',$.alerts._reposition);break;
case false:
...
//jPrompt=function(message,title,value,callback){$.alerts.prompt(message,title,value,callback);};
jPrompt=function(message,title,value,callback){
			$.alerts.prompt(message,title,value,callback);
		};

jIframe=function(url,title,width,height){$.alerts.iframe(url,title,width,height);};


jChoose=function(html,title,width,height,callback){$.alerts.choose(html,title,width,height,callback);};




jHtml=function(html,title,width,height){$.alerts.html(html,title,width,height);};})(jQuery);