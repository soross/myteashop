...
if($_SESSION['WEB_AAMS_USER_LOGIN_UID_SESSION']==1){
	if(isset($_POST[task]) && "addAamsUser"==$_POST[task]){
		if($_POST[password]==$_POST[password2]){
			$db->query("select * from aams_user where username='".$_POST[username]."'");
			$cnt = $db->db_num_rows();
			if($cnt<1){
				$db->query("insert into aams_user(username,password,realname,create_date) values('".$_POST[username]."','".md5($_POST[password])."','".$_POST[realname]."',now())");
...
if(isset($_GET[task]) && "toUpdateAamsUser"==$_GET[task]){
	if(isset($_GET[aamsid]) && !empty($_GET[aamsid])){
		if($_SESSION['WEB_AAMS_USER_LOGIN_UID_SESSION'] == $_GET[aamsid] || $_SESSION['WEB_AAMS_USER_LOGIN_UID_SESSION']==1){
			$db->query("update aams_user set password='".md5($_GET[password])."' where id ='".$_GET[aamsid]."'" );
