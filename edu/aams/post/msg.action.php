...
if(isset($_GET[task]) && "updateState"==$_GET[task]){
	$db->query("update onlinereg  set state='1' where id= '$_GET[msgid]'");
	//mode=info
	if(isset($_GET[mode])&&"info"==$_GET[mode]){
		echo "<script>alert('在线注册处理成功!');location.href='../msginfo.php?msgid=$_GET[msgid]';</script>";
	}else{
		echo "<script>alert('在线注册处理成功!');location.href='../msg.php?page=$_GET[cpage]';</script>";
	}
}else if(isset($_GET[task]) && "updateStateddddd"==$_GET[task]){
