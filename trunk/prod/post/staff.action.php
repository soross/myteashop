...
if(isset($_POST[task]) && "addStaff"==$_POST[task]){
	...
	if($cnt<1){
		...
		echo "<script>if(confirm('新闻新增成功,是否继续新增?')){location.href='../addstaff.php';}else{location.href='../stafflist.php';}</script>";
	}else{
	...
	echo "<script>alert('删除成功?');location.href='../stafflist.php';</script>";
	   	...
	   	//print_r(custid);
		$db->query("update staff set staffid='$_POST[staffid]',staffname='$_POST[staffname]',gwid='$_POST[gwid]',tell='$_POST[tell]' where id='$_POST[id]'");
