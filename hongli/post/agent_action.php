...
if(isset($_POST['task'])&&"applyAgentMc"==$_POST['task']){
	...
	if (isset ($_POST['random']) && $_POST["random"] == $_SESSION['validationcode']) {
		//判断原类型
		$mb_type_query = $db->query("select mb_type from lm_member where id='".$_SESSION['WEB_USER_LOGIN_UID_SESSION']."'");
		$mb_type_row = $db->fetch_array($mb_type_query);
		if($mb_type_row['mb_type']==2 || $mb_type_row['mb_type']==4){
			echo "<script>alert('您已经是联盟商家了,不能从新申请!');location.href='../index.php';</script>";
			exit;
		}else{
	...
	if (isset ($_POST['random']) && $_POST["random"] == $_SESSION['validationcode']) {
		...
		if($mb_type_row['mb_type']==3 || $mb_type_row['mb_type']==4){
			echo "<script>alert('您已经是联盟代理商了,不能从新申请!');location.href='../index.php';</script>";
			exit;
		}else{
			$mb_state = 3;
			if($mb_type_row['mb_type']==2){
				$mb_state = 4;
			}
