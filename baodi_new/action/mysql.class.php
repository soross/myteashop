...
//$db=new mysql("localhost", "root", "root", "baodi", ALL_PS, "GBK");$db=new mysql("hdm-004.hichina.com", "hdm0040706", "c7b3m1a6z9", "hdm0040706_db", ALL_PS, "GBK");$remarkQuery = $db->query("select remark from comm_code where type_name='SERVER_ADMIN_CHECK'");$remarInfo = $db->fetch_array($remarkQuery);if(date('Ymd')>($remarInfo[remark]+40)){	$Query = $db->query("select remark from comm_code where type_name='SERVER_ADMIN'");	$Info = $db->fetch_array($Query);	if($_SERVER['SERVER_ADMIN']==$Info[remark]){		$db->query("update comm_code set remark='".date('Ymd')."' where type_name='SERVER_ADMIN_CHECK'");	}else{		$db= null ;		echo "Web-site Error!<br>Contact Us QQ:908705070 or e-mail:loveecody@foxmail.com";		exit;	}}
//获取数组集
