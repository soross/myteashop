 ...
 *       <ul>
 *       <li><a
 *       href="http://rowactions.extjs.eu/rowactions.tar.bz2">rowactions.tar.bz2</a></li>
 *       <li><a
...
Ext.ux.grid.RowActions = function(config) {
	...
	this.addEvents(
			/**
			 * @event beforeaction Fires before action event. Return false to
			 *        cancel the subsequent action event.
			 * @param {Ext.grid.GridPanel}
			 *            grid
			 * @param {Ext.data.Record}
			 *            record Record corresponding to row clicked
			 * @param {String}
			 *            action Identifies the action icon clicked. Equals to
			 *            icon css class name.
			 * @param {Integer}
			 *            rowIndex Index of clicked grid row
			 * @param {Integer}
			 *            colIndex Index of clicked grid column that contains
			 *            all action icons
			 */
			'beforeaction'
			/**
...
Ext.extend(Ext.ux.grid.RowActions, Ext.util.Observable, {
	...
	init : function(grid) {
		...
		// calculate width
		if (this.autoWidth) {
			this.width = this.widthSlope * this.actions.length
					+ this.widthIntercept;
			this.fixed = true;
		}
		...
		if (view.groupTextTpl && this.groupActions) {
			view.interceptMouse = view.interceptMouse.createInterceptor(
					function(e) {
						if (e.getTarget('.ux-grow-action-item')) {
							return false;
						}
					});
			view.groupTextTpl = '<div class="ux-grow-action-text">'
					+ view.groupTextTpl
					+ '</div>'
					+ this.processActions(this.groupActions, this.tplGroup)
							.apply();
		}
					...
					var o = {
						...
						qtip : a.qtipIndex
								...
								: (a.tooltip || a.qtip
										? a.tooltip || a.qtip
										: ''),
						text : a.textIndex ? '{' + a.textIndex + '}' : (a.text
								? a.text
								: ''),
						hide : a.hideIndex
								? '<tpl if="'
										+ a.hideIndex
										+ '">'
										+ ('display' === this.hideMode
												? 'display:none'
												: 'visibility:hidden')
										+ ';</tpl>'
								: (a.hide ? ('display' === this.hideMode
										? 'display:none'
										: 'visibility:hidden;') : ''),
						align : a.align || 'right',
	...
	onClick : function(e, target) {
		...
		if (false !== row && false !== col && false !== action) {
			...
			if (this.callbacks && 'function' === typeof this.callbacks[action]) {
				this.callbacks[action](this.grid, record, action, row.rowIndex,
						col);
			}

			// fire events
			if (true !== this.eventsSuspended
					&& false === this.fireEvent('beforeaction', this.grid,
							record, action, row.rowIndex, col)) {
				return;
			} else if (true !== this.eventsSuspended) {
				this.fireEvent('action', this.grid, record, action,
						row.rowIndex, col);
			}
		...
		if (t) {
			...
			var groupId = group
					? group.id.replace(/ext-gen[0-9]+-gp-/, '')
					: null;
			...
			action = t.className.replace(
					/ux-grow-action-item (ux-action-right )*/, '');
			...
			// fire events
			if (true !== this.eventsSuspended
					&& false === this.fireEvent('beforegroupaction', this.grid,
							records, action, groupId)) {
				return false;
			}
