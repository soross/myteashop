...
$pagesize = 10;//一页显示多少条
//分页
$queryTotal = $db->query("select id from orderitem");
$total = $db->db_num_rows($queryTotal);
pageft($total, $pagesize);
if ($firstcount < 0) $firstcount = 0;


$orderRow = getListBySql("select oi.*,(select c.custname from customer c where c.id = oi.custid)as custname from orderitem oi order by id desc limit $firstcount, $displaypg",$db);
