...
<body>
	...
	<table width="100%" border="0" align="center" cellpadding="2"
		...
		<tr>
			<td height=25 class="pl20">
			登入帐号:<html:text property="userid"  size="15" />(模糊)
			</td>
			<td height=25 class="pl20">
			开始时间:	<html:text property="startCeateDates" onfocus="javascript:WdatePicker({dateFmt:'yyyy-MM-dd HH:mm:ss'});"
						styleId="startCeateDates" styleClass="Wdate" readonly="true" size="17"  />
			</td>
			<td height=25 class="pl20">
			结束时间:	<html:text property="endCeateDates" onfocus="javascript:WdatePicker({dateFmt:'yyyy-MM-dd HH:mm:ss'});"
		...
		<logic:present name="logList">
			...
			<logic:iterate id="user" name="logList">
					<tr align="center">
						...
						<td>
							<a href="${pageContext.request.contextPath}/admin/user.do?task=deleteUserLoginLog&modfunid=900&id=${user.id}" onclick="return confirm('是否确定删除!');"><img src="${pageContext.request.contextPath}/admin/images/del.gif" border="0">[删除]</a>
						</td>
					</tr>
			</logic:iterate>
		</logic:present>
