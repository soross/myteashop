...
//热点推荐
$newList = getListBySql("select id,title from news where type_id in(select id from type where id='1' or pid='1') order by create_date desc limit 0,8",$db);
$smarty->assign('newlist',$newList);

//新闻中心
$hotList = getListBySql("select id,title from news where type_id in(select id from type where id='15' or pid='15') order by create_date desc limit 0,8",$db);
$smarty->assign('hotlist',$hotList);

//自学考试2
$selfList = getListBySql("select id,title from news where type_id in(select id from type where id='2' or pid='2') order by create_date desc limit 0,8",$db);
