...
$("a.thickbox").click(function(){
  var t = this.title;
  TB_show(t,this.href,this.name);
  this.blur();
  return false;
});
}

function TB_show(caption, url, name) { //function called when the user clicks on a thickbox link
	try {
		$("body")
		...
		$("#TB_overlay").show();
		var urlString = /.jpg|.jpeg|.png|.gif|.html|.php|.htm/g;
		...
		if(urlType == '.jpg' || urlType == '.jpeg' || urlType == '.png' || urlType == '.gif'){//code to show images
			...
			imgPreloader.onload = function(){
				...
				if(imgPreloader.width > x) {
					imgPreloader.height = imgPreloader.height * (x/imgPreloader.width)*1;
					imgPreloader.width = x*1;
					if(imgPreloader.height > y) {
						imgPreloader.width = imgPreloader.width * (y/imgPreloader.height)*1;
						imgPreloader.height = y*1;
					}
				}
				...
				$("#TB_window").append("<div id='TB_closeAjaxWindow'><a href='#' id='TB_closeWindowButton'>关闭</a></div><div id='TB_ajaxContent'><img id='TB_Image' src='"+url+"' width='"+imgPreloader.width+"' height='"+imgPreloader.height+"' alt='"+caption+"'/></div>");
		...
		if(urlType == '.htm' || urlType == '.html' || urlType =='.php'){//code to show html pages
			...
			$("#TB_window").append("<div id='TB_closeAjaxWindow'><a href='#' id='TB_closeWindowButton'>关闭</a></div><div id='TB_ajaxContent' style='width:"+ajaxContentW+"px;height:"+ajaxContentH+"px;text-align:center;'></div>");
			$("#TB_closeWindowButton").click(TB_remove);
